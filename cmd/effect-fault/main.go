...
func main() {
	...
	if url == "" { panic("DAR_WORKER_POSTGRES_URL is required") }
	...
	for range *workers {
		...
		go func() {
			...
			for range jobs {
				...
				if queryErr != nil { failed.Add(1); continue }
				inserted.Add(command.RowsAffected())
			}
		}()
	}
	for range *attempts { jobs <- struct{}{} }
	close(jobs)
	group.Wait()
	var persisted int64
	if err = pool.QueryRow(ctx, "select count(*) from effect_commits where tenant_id = $1 and run_id = $2", tenantID, runID).Scan(&persisted); err != nil { panic(err) }
	...
	report := report{
		...
		Machine: machine{OS: runtime.GOOS, Arch: runtime.GOARCH, CPUs: runtime.NumCPU(), GoVersion: runtime.Version()}, CompletedAt: time.Now().UTC(),
	}
	if err = os.MkdirAll(filepath.Dir(*output), 0o755); err != nil { panic(err) }
	encoded, _ := json.MarshalIndent(report, "", "  ")
	if err = os.WriteFile(*output, encoded, 0o644); err != nil { panic(err) }
