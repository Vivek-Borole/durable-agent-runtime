...
const demoWorkflow = {
  ...
  steps: [
    { kind: "tool", tool: "mock_data_read", sideEffect: false },
    {
      ...
      input: "summarize fixture findings",
    },
...
export function App() {
  ...
  const [message, setMessage] = useState(
    "Connect locally, then create a synthetic approval-gated run.",
  );
  ...
  async function request(path: string, init: RequestInit = {}) {
    const requestHeaders = { ...headers, ...(init.headers ?? {}) } as Record<
      string,
      string
    >;
  ...
  async function createDemoRun() {
    ...
    try {
      const workflow = await request("/v1/workflows", {
        ...
        body: JSON.stringify({
          ...demoWorkflow,
          name: `research-demo-${crypto.randomUUID().slice(0, 8)}`,
        }),
      });
      setMessage(
        "Queueing an idempotent run. No provider key or real external action is used.",
      );
      const created = await request("/v1/runs", {
        ...
        body: JSON.stringify({
          ...
          input: { query: "synthetic safe fixture" },
        }),
      });
      setRun(created.run);
      setMessage(
        "Run queued. The worker will stop at approval before the mock side effect.",
      );
    } catch (error) {
      setMessage(
        error instanceof Error ? error.message : "Could not create run",
      );
    } finally {
      setBusy(false);
    }
  ...
  async function approve() {
    ...
    setBusy(true);
    try {
      const result = await request(`/v1/runs/${run.id}/approve`, {
        method: "POST",
      });
      setRun(result.run);
      setMessage(
        "Approval recorded. The run is safely re-queued at the next step.",
      );
    } catch (error) {
      setMessage(
        error instanceof Error ? error.message : "Could not approve run",
      );
    } finally {
      setBusy(false);
    }
  ...
  async function cancel() {
    ...
    setBusy(true);
    try {
      const result = await request(`/v1/runs/${run.id}/cancel`, {
        method: "POST",
      });
      setRun(result.run);
  ...
  return (
    <main>
      <header>
        <p className="eyebrow">Developer infrastructure / v0.2 candidate</p>
        <h1>Durable Agent Runtime</h1>
        <p className="lede">
          Recoverable agent workflows with explicit approval, tenant boundaries,
      ...
      <section className="card connection">
        ...
        <button onClick={() => void createDemoRun()} disabled={busy}>
          {busy ? "Working…" : "Create synthetic run"}
        </button>
      </section>
      ...
      <section className="grid">
        ...
        <article className="card">
          ...
          <p>
            {run
              ? `Run ${run.id.slice(0, 8)} · budget ${(run.budgetCents / 100).toFixed(2)} USD`
              : "No run created yet"}
          </p>
          {run && (
      ...
      <section className="card timeline">
        ...
        {run?.events.length ? (
          ...
          <p>
            Events will appear here. Provider credentials are never sent to this
            view.
          </p>
        )}
      </section>
