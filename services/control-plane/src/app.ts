...
export function createApp(
  store: RuntimeStore = new InMemoryDurableStore({
    tenantId: "demo-tenant",
    apiKey: "replace-with-a-long-local-key",
  }),
  ...
  void app.register(cors, {
    origin: (origin, callback) => {
      const allowed = (
        process.env.CONSOLE_ORIGIN ?? "http://127.0.0.1:5173"
      ).split(",");
  ...
  app.addHook("preHandler", async (request, reply) => {
    ...
    const principal = tenant
      ? await store.authenticate(tenant, key)
      : undefined;
  ...
  app.addHook("onRequest", async (request) => {
    request.telemetrySpan = requestTracer().startSpan("http.request", {
      attributes: {
        ...
        "url.path": request.routeOptions.url ?? request.url.split("?")[0],
      },
    });
  });
  ...
  app.addHook("onResponse", async (request, reply) => {
    ...
    request.telemetrySpan?.setAttribute(
  ...
  app.post("/v1/workflows", async (request, reply) => {
    ...
    if (!parsed.success)
      return reply
        .code(400)
        .send({ error: "invalid_workflow", details: parsed.error.issues });
  ...
  app.post("/v1/runs", async (request, reply) => {
    ...
    if (!parsed.success)
      return reply
        .code(400)
        .send({ error: "invalid_run", details: parsed.error.issues });
    try {
      const { providerCredential, ...persisted } = parsed.data;
      const providerCredentialHandle = providerCredential
        ? broker.issue(providerCredential)
        : undefined;
      const result = await store.createRun(
        principal,
        { ...persisted, providerCredentialHandle },
        key,
      );
      if (result.replayed && providerCredentialHandle)
        broker.discard(providerCredentialHandle);
      runCreations.inc({ replayed: String(result.replayed) });
      return reply
        .code(result.replayed ? 200 : 201)
        .send({ run: result.run, replayed: result.replayed });
    } catch (error) {
      const message =
        error instanceof Error ? error.message : "run_creation_failed";
      if (message.includes("quota")) quotaRejections.inc();
      return reply
        .code(
  ...
  app.post("/internal/credentials/:handle/consume", async (request, reply) => {
    ...
    if (
      !internalToken ||
      request.headers["x-runtime-internal-token"] !== internalToken
    )
      return reply.code(401).send({ error: "internal_unauthenticated" });
    const credential = broker.consume(
      (request.params as { handle: string }).handle,
    );
    return credential
      ? reply.send({ credential })
      : reply.code(404).send({ error: "provider_credential_unavailable" });
  });
  ...
  app.post("/v1/runs/:runId/approve", async (request, reply) => {
    ...
    if (run.state !== "awaiting_approval")
      return reply.code(409).send({ error: "run_not_awaiting_approval" });
    const updated = await store.transition(
      principal,
      runId,
      "queued",
      runEvent("approved", "Approval recorded; run re-queued"),
    );
  ...
  app.post("/v1/runs/:runId/cancel", async (request, reply) => {
    ...
    if (
      !parsed.success ||
      ["succeeded", "failed", "cancelled", "uncertain"].includes(parsed.data)
    ) {
    ...
    const updated = await store.transition(
      ...
      runId,
      "cancelled",
      runEvent("cancelled", "Cancellation requested"),
    );
    cancellations.inc();
    return reply.send({ run: updated });
  });
