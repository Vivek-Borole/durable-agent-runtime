...
const systemDatabaseUrl = process.env.DAR_TEST_ADMIN_POSTGRES_URL ?? databaseUrl;
const workerDatabaseUrl = process.env.DAR_WORKER_POSTGRES_URL ?? systemDatabaseUrl;
...
integration("PostgreSQL durable store", () => {
  ...
  afterAll(async () => {
    await systemPool.query("delete from tenants where slug = any($1::text[])", [[tenantSlug, secondTenantSlug]]);
    await store.close();
  ...
  it("keeps outbox and worker lease columns unavailable to the control-plane role", async () => {
    await expect(store.pool.query("select * from workflow_outbox limit 1")).rejects.toThrow("permission denied");
    await expect(store.pool.query("update workflow_runs set lease_owner = 'forbidden' where false")).rejects.toThrow("permission denied");
  });
  ...
  it.skipIf(!natsUrl)("publishes a committed outbox entry to JetStream", async () => {
    ...
    await systemPool.query("update workflow_outbox set subject = 'dar.test.run.queued' where tenant_id = $1 and published_at is null", [principal.tenantId]);
    ...
    try {
      const publisher = new OutboxPublisher(workerStore, connection.jetstream());
  ...
  it("requeues an approved run at the step after its approval and emits a new outbox item", async () => {
    ...
    await expect(systemPool.query<{ current_step: number }>("select current_step from workflow_runs where id = $1", [created.run.id])).resolves.toMatchObject({ rows: [{ current_step: 1 }] });
    await expect(systemPool.query<{ count: string }>("select count(*) from workflow_outbox where run_id = $1", [created.run.id])).resolves.toMatchObject({ rows: [{ count: "2" }] });
  });
  ...
  it("enforces active run limits under the tenant transaction lock", async () => {
    ...
    await systemPool.query("update tenant_runtime_limits set max_active_runs = 1, max_runs_per_day = 1000 where tenant_id = $1", [principal.tenantId]);
